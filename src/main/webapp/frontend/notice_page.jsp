        ...
        $.get("/api/notes", function (notes) {
            ...
            $.each(notes, function (index, note) {
                $("#scrl").append(
                    ...
                    + "<div class=\"title\">" + note.topic + "</div>"
                    + "<div class=\"text\">" + note.content + "</div>"
                    + "<div onclick='deleteNote(this);' id=\"" + note.id + "\" class=\"delete\">Delete this notice</div>"
                    + "</div>"
                );
...
<div class="basis">  <!-- ТІНЬ -->
    ...
    <div class="board_scroll">  <!-- БОРДЕР З КНОПКАМИ -->
        <div class="left_button" onclick="showHideModal(true)">
            <a href="#openModal"><font>Add alert</font></a>
        </div>
        <!-- КНОПКА ДОДАТИ ОГОЛОШЕННЯ(ПРИ НАТИСКУ ГЕНЕРУЄТЬСЯ БЛОК З КЛАСОМ  advertisement , МОЖЕШ ДО НЬОГО ДОДАТИ ID) -->
        <div onclick="clearTable()" class="right_button">Clear board</div>
        <!-- КНОПКА ВИДАЛИТИ УСІ ОГОЛОШЕННЯ(ПРИ НАТИСКУ ВИДАЛЯЮТЬСЯ УСІ ОГОЛОШЕННЯ З КЛАСОМ advertisement АБО ТВОЇМ ID) -->
    </div>


    <div id="openModal" class="modalDialog">
        <div>
            <input id="modalTopic" class="input_style1"
                   style="	margin-top:2vw;" type="text"
                   name="" required placeholder="Write a notification header"
                   pattern="[A-Za-zА-Яа-яЁё]" maxlength="35">
            <textarea id="modalContent" class="input_style2"
                      style="	margin-top: 7vw;"
                      type="text" name="" required
                      placeholder="Describe your notification"
                      pattern="[A-Za-zА-Яа-яЁё0-9]" maxlength="300"></textarea>
            <input class="input1" type="submit" value="Save"
                   onclick="addNote()" alt="Input">
            <input class="input2" type="submit" value="Close"
                   onclick="showHideModal(false)" alt="Input">
        </div>
    </div>


    <div id="scrl" class="scroll_block">
        <!-- БЛОК В ЯКОМУ САМЕ БУДУТЬ ПОЯВЛЯТИСЬ САМЕ ЦІ ПОВІДОМЛЕННЯ З КЛАСОМ advertisement  АБО ТВОЇМ ID  -->


        <%--<div class="advertisement" id="">--%>
        <%--<!--   ОСЬ БЛОК З КЛАСОМ advertisement (ТИ МОЖЕШ В НЬОМУ ВКАЗАТИ  ID ДЛЯ СКРИПТІВ) САМЕ ЙОГО ТРЕБА ГЕНЕРУВАТИ ПРИ НАЖАТІ НА КНОПКУ  -->--%>
        <%--<div class="title">INTRODUCTION TEXT FOR THE TITLE</div>--%>
        <%--<!-- БЛОК В ЯКИЙ МАЄШ ЗАЛИТИ ЗАГОЛОВОК ПІСЛЯ ВВЕДЕНЯ  -->--%>
        <%--<div class="text">INTRODUCTION TEXT FOR THE MESSAGE</div>--%>
        <%--<!-- БЛОК В ЯКИЙ МАЄШ ЗАЛИТИ ТЕКСТ ОГОЛОШЕННЯ ПІСЛЯ ВВЕДЕНЯ  -->--%>
        <%--<div class="delete">Delete this notice</div>--%>
        <%--<!-- КНОПКА ДЛЯ ВИДАЛЕННЯ САМЕ ЦЬОГО БЛОКУ, А НЕ УСІХ ВІДРАЗУ -->--%>
        <%--</div>--%>
